...
function getAll() {
  requestCRUD("GET").then((response) =>
    response ? showDatos(response) : showAlert()
  );
}

function getDatos(id) {
  requestCRUD("GET", { id }).then((response) =>
    response ? showDatos([response]) : showAlert()
  );
}

function deleteDatos(id) {
  requestCRUD("DELETE", { id }).then((response) =>
    response ? getAll() : showAlert()
  );
}

function postDatos(data) {
  requestCRUD("POST", data).then((response) =>
    response ? getAll() : showAlert()
  );
}

function putDatos(data) {
  requestCRUD("PUT", data).then((response) =>
    response ? getAll() : showAlert()
  );
}
  ...
  document.getElementById("btnPut").addEventListener("click", () => {
    ...
    requestCRUD("GET", { id }).then((response) => {
      if (response) {
        ...
        let dataModal = new bootstrap.Modal(
          document.getElementById("dataModal")
        );
        dataModal.show();
        document
          .getElementById("btnSendChanges")
